...
const defaultNodeRenderers: RenderNode = {
    ...
    [BLOCKS.EMBEDDED_ASSET]: (node) => {
        ...
        if (node.data.target.fields.file.contentType.includes('image/')) {
            const src =
                typeof node.data.target.fields.file.url === 'string'
                    ? node.data.target.fields.file.url
                    : '';
            const alt =
                typeof node.data.target.fields.title === 'string'
                    ? node.data.target.fields.title
                    : '';
            return `<img src=${attributeValue(src)} alt=${attributeValue(
                alt
            )} />`;
        }
        // embed PDF in page
        if (
            ...
            const src =
                typeof node.data.target.fields.file.url === 'string'
                    ? node.data.target.fields.file.url
                    : '';
            return `<iframe src=${attributeValue(
                src
            )} width="100%" height="500px"></iframe><a href=${attributeValue(
                src
            )}>${node.data.target.fields.title} ${downloadIcon}</a>`;
        }
        const href =
            typeof node.data.target.fields.file.url === 'string'
                ? node.data.target.fields.file.url
                : '';
        return `<a href=${attributeValue(href)}>${
            node.data.target.fields.title
        } ${downloadIcon}</a>`;
    },
...
function nodeListToHtmlString(
    ...
    return nodes
        .map<string>((node) =>
            nodeToHtmlString(node, { renderNode, renderMark })
        )
        .join('');
}
...
function nodeToHtmlString(
    ...
    if (helpers.isText(node)) {
        ...
        const nextNode: Next = (nodes) =>
            nodeListToHtmlString(nodes, { renderMark, renderNode });
        // @ts-ignore
